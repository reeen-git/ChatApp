...
struct ChatView: View {
    ...
    @FocusState private var isFoucused
    ...
    var body: some View {
        VStack(spacing: 0) {
            GeometryReader { reader in
                ScrollView {
                    ScrollViewReader { scroolReader in
                        getMessageView(viewWidth: reader.size.width)
                            .padding(.horizontal)
                            .onChange(of: messageIdscrool) { _ in
                                if let messageID = messageIdscrool {
                                    scrollTo(messageId: messageID, shouldAnimate: true, scrollReader: scroolReader)
                                }
                            }
                            .onAppear {
                                if let messaId = chat.message.last?.id {
                                    scrollTo(messageId: messaId, anchor: .bottom, shouldAnimate: false, scrollReader: scroolReader)
                                }
                            }
                    }
                }
        ...
        .navigationBarItems(leading: navBarLeadingBtn, trailing: navBarTradingBtn)
        .onAppear {
    ...
    var navBarTradingBtn: some View {
        HStack {
            Button(action: {}) {
    ...
    func getMessageView(viewWidth: CGFloat) -> some View {
        LazyVGrid(columns: culumns, spacing: 0, pinnedViews: [.sectionHeaders]) {
